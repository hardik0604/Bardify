...
import { Eraser, Download, Copy, Sparkles } from 'lucide-react';

const ActionButtons = memo(function ActionButtons({ onClear, onCopy, onSubmit, onDownload, isTranslating, hasInput, hasOutput }) {
  return (
    <div className="grid grid-cols-2 sm:flex sm:flex-row items-stretch sm:items-center justify-center sm:justify-end gap-3 sm:gap-4 mt-6">
      ...
      <motion.button
        ...
        className="col-span-1 flex-1 px-2 sm:px-6 py-3 rounded-2xl bg-bg-surface border border-border-glass text-text-muted hover:text-brand-gold hover:bg-bg-card hover:border-brand-gold/30 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold disabled:opacity-30 disabled:cursor-not-allowed uppercase tracking-widest text-[10px] sm:text-sm font-bold flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 shadow-lg"
